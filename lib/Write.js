...
const indexTypes = ['index.js', 'index.ts'];
...
module.exports = class Write {
    ...
    write(bemString, types, simulate) {
        ...
        const indexType = types.find((type) => indexTypes.includes(type));

        if (indexType) {
            if (!this._templates[indexType]) {
                console.error(`No templates for "${indexType}" file type`);

                return;
            }

            const filePath = path.join(directoryPath, indexType);

            if (!fs.existsSync(filePath)) {
                if (!simulate) {
                    const content = this._generateFileContent(bemEntity, indexType);
        ...
        types.filter((type) => !indexTypes.includes(type)).forEach((type) => {
