 ...
 * @property {string} wordPattern - Word pattern regex source
 */

/**
 * @type {{block: 'block', blockMod: 'blockMod', elem: 'elem', elemMod: 'elemMod'}}
 */
export const BEM_TYPES = {
    block: 'block',
    blockMod: 'blockMod',
    elem: 'elem',
    elemMod: 'elemMod',
...
export function createBemNaming(options) {
    ...
    return {
        ...
        typeOf(entity) {
            ...
            if (entity.elem) {
                if (isMod) return BEM_TYPES.elemMod;
                if (!modName) return BEM_TYPES.elem;
            }

            if (isMod) return BEM_TYPES.blockMod;
            if (!modName) return BEM_TYPES.block;
