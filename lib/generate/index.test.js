...
import config from '../../bemg/config.json' with { type: 'json' };
...
const bemg = {
    ...
    templates: {
        ...
        'template_namedItem.ts': '${block} ${Block} ${entity} ${camelNamedItem} ${pascalNamedItem} ${kebabNamedItem}',
        'block.i18n': {
            'block.en.json': '${block} ${Block} ${entity}',
        },
        'block.hooks': {
            'use-namedItem.ts': '${block} ${Block} ${entity} ${camelNamedItem} ${pascalNamedItem} ${kebabNamedItem}',
        },
    },
};
...
describe('generate', () => {
    ...
    test('Creates nested folders', () => {
        ...
        generate({
            targetPath: '/block',
            types: ['block.i18n/block.en.json'],
        });

        expect(fs.__getMockFiles()['/block/block.i18n/block.en.json']).toEqual('block Block block');
    });
    ...
    test('Creates nested folders in new block when name is provided', () => {
        ...
        expect(fs.__getMockFiles()['/newblock/newblock.i18n/newblock.en.json']).toEqual('newblock Newblock newblock');
    });
    ...
    test('Creates hook for a block', () => {
        ...
        generate({
            targetPath: '/block',
            types: ['block.hooks/use-namedItem.ts'],
            items: ['namedItem'],
        });

        expect(fs.__getMockFiles()['/block/block.hooks/use-named-item.ts']).toEqual(
            'block Block block namedItem NamedItem named-item',
        );
    });

    test('Creates hook in new block folder when name is provided', () => {
        fs.__setMockFiles({
            bemg,
        });
        ...
        expect(fs.__getMockFiles()['/newblock/newblock.hooks/use-named-item.ts']).toEqual(
            'newblock Newblock newblock namedItem NamedItem named-item',
        );
    });

    test('Resolves hook template by alias', () => {
        fs.__setMockFiles({
            bemg,
            block: {},
        });
    ...
    test('Normalizes named item passed in kebab-case', () => {
        fs.__setMockFiles({
            bemg,
            block: {},
        });
    ...
    test('Creates named item file from a flat template', () => {
        fs.__setMockFiles({
            bemg,
            block: {},
        });
        ...
        expect(fs.__getMockFiles()['/block/named-item.ts']).toEqual('block Block block namedItem NamedItem named-item');
    });

    test('Returns named item in generation result', () => {
        fs.__setMockFiles({
            bemg,
            block: {},
        });
        ...
        expect(result).toEqual([
            {
                creatingEntityString: 'namedItem',
                creatingEntityType: 'namedItem',
                creatingEntity: { block: 'block' },
                templateType: 'block.hooks/use-namedItem.ts',
                creatingFilePath: '/block/block.hooks/use-named-item.ts',
            },
        ]);
    });

    test('Skips named item when current entity is not a block', () => {
        fs.__setMockFiles({
            bemg,
            block: {
                __elem: {},
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith(
            'Skip "namedItem". Named items can only be created for a block, but "block__elem" has type elem.',
        );
        expect(fs.__getMockFiles()['/block/__elem/block.hooks/use-named-item.ts']).toBeUndefined();

        consoleErrorSpy.mockRestore();
    });

    test('Skips named item with invalid name', () => {
        fs.__setMockFiles({
            bemg,
            block: {},
        });
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('Skip "named item!". Not valid named item name.');
        expect(fs.__getMockFiles()['/block/block.hooks/use-named-item.ts']).toBeUndefined();

        consoleErrorSpy.mockRestore();
    });

    test('Reports missing named item template', () => {
        fs.__setMockFiles({
            bemg,
            block: {},
        });
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('No templates for "namedItem" in "tsx" file type');
        expect(fs.__getMockFiles()['/block/blocknamedItem.tsx']).toBeUndefined();
        expect(fs.__getMockFiles()['/block/named-item.tsx']).toBeUndefined();

        consoleErrorSpy.mockRestore();
    });

    test('Creates bem items and named items together', () => {
        fs.__setMockFiles({
            bemg,
            block: {},
        });
