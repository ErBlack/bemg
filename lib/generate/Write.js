...
const capitalize = (string) =>
    string && typeof string === 'string' && string.slice(0, 1).toUpperCase() + camelCase(string.slice(1));
...
module.exports = class Write {
    ...
    write(bemString, types, simulate) {
        ...
        types.forEach((type) => {
            ...
            const isDirectoryTemplate = type.includes('/') && this._bemNaming.typeOf(bemEntity) === 'block';
            const fileName = isDirectoryTemplate
                ? this._getDirectoryPath(type, bemEntity)
                : this._getBemFilePath(bemString, type);
    ...
    _getBemFilePath(bemString, type) {
        return `${bemString}.${type}`;
    }
    _getDirectoryPath(type, entity) {
        return type.replace(/block/g, entity.block);
    }
    /**
    ...
    _buildTemplateContext(entity) {
        return Object.assign(
            {
                user: user,
                entity: this._bemNaming.stringify(entity),
                Block: capitalize(entity.block),
                Elem: capitalize(entity.elem),
                ModName: capitalize(entity.modName),
                ModVal: typeof entity.modVal === 'string' ? capitalize(entity.modVal) : entity.modVal,
            },
            entity,
        );
    }
