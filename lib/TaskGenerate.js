...
module.exports = class TaskGenerate {
    constructor(targetPath) {
        ...
        const pathDirname = stats.isDirectory() ? absolutePath : path.dirname(absolutePath);

        const configPath = getConfigPath(pathDirname);

        const { aliases, naming } = require(configPath)

        this._bemNaming = createBemNaming(naming);

        if (!this._bemNaming.validate(this._bemString)) {
        ...
        this._write = new Write(
            this._bemRoot(pathDirname, this._bemNaming.typeOf(bemEntity)),
            this._bemNaming,
            aliases,
            naming
        );
